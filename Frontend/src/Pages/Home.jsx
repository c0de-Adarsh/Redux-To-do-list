...
const Home = () => {
  return (
    <>
    <div className='min-h-screen flex justify-center pt-20 bg-gradient-to-b from-purple-900 to-blue-500'>
      <div className='flex flex-col items-center py-3 md:w-[100%] px-3 w-1/1'>
        <h1 className='md:text-4xl text-center text-3xl font-bold text-white'>Welcome to ViTo ToDo's - Empowering Your Productivity</h1>

        <h4 className='text-xl py-3 underline font-bold text-white'>Unleash Your Inner Hero</h4>
       <div className='py-3'><Link className='font-bold text-white bg-gradient-to-r from-blue-500 to-purple-500 py-2 px-3' to={'/login'}>Set' Your todo's Now</Link></div>
       <div className='text-center py-10 gap-5 flex flex-col text-white font-semibold'>
        <div className='flex flex-row md:px-44 px-7'>
          <div><p className='font-bold text-xl md:px-11'>Are you ready to take charge of your tasks and conquer your to-do list? Look no further! DoItHero is here to help you become the hero of your own productivity journey.</p></div>
        </div>
       ...
       <div className='py-16 flex gap-4 flex-col md:px-56 px-8 text-white font-semibold'>
        ...
        <p><span><Link className='font-bold underline text-xl' to={'/signup'}> Sign up </Link></span>for DoItHero now and embark on your journey to productivity greatness!</p>
       </div>
      </div>
    </div>
    </>
